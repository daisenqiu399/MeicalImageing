...
app.post('/api/ai/chat', async (req, res) => {
  try {
    const { message, conversationHistory, studyContext, image } = req.body;
    ...
    if (image) {
      // ⚠️ DeepSeek API doesn't support images directly
      // We'll send the text message with a note about the image
      console.log('⚠️ Image uploaded but DeepSeek API is text-only');
      console.log('📝 Image data length:', image.length);

      // Inform user about limitation and describe what would be analyzed
      userMessage.content = `[Image uploaded: ${message ? message : 'Please analyze this medical image'}]\n\nNote: I can see that you've uploaded an image, but I'm a text-based AI assistant. I cannot view or analyze images directly. However, if you describe what you see in the image, I can help explain medical concepts, anatomy, or answer questions based on your description.`;
    } else {
      // Text-only message
    ...
    console.log('📝 Sending messages to DeepSeek:', JSON.stringify(messages, null, 2).substring(0, 500) + '...');
